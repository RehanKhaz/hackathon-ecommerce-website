    ...
    <div className='overflow-x-hidden'>
      ...
      <div className='cp flex flex-col items-center new:flex-row justify-between gap-4 py-[3rem]'>
        <div className=' w-full max-md:mt-[3em] lg:w-[70%] flex  md:gap-[6em] flex-col  items-center justify-between '>
          <ul className=' py-5 new:ms-[5em] flex flex-row w-[90%]  lg:w-full  items-center px-4   md:gap-[4rem] max-md:justify-between md:justify-center 
         bg-[#FFF9E5]'>
            {
          ...
          <div className='flex flex-col max-md:mt-[5em] gap-y-[4em]  w-[100%] items-center '>
            ...
            {cartItems.length == 0 ? <p className='text-xl font-medium tracking-wider'>Your cart is Empty.</p> : cartItems.map((item, index) => {
              return <div key={index} className=' mt-[-3rem] w-full md:mt-0 flex flex-row    items-center px-2 gap-3  new:gap-5 justify-between    '>
                ...
                <p className={`text-[#9F9F9F]'  lg:ms-[-1em]  text-sm sm:text-[1em] md:text-lg`}>{item.name} </p>
                <p className='text-[.7em] sm:text-[.9em] md:text-lg'><span className='max-sm:hidden'>Rs</span> {item.price} </p>
                <p className='border text-[.9em] md:text-lg border-black px-3 py-2 rounded-lg' >{item.quantity} </p>
                <p className='text-[.7em] sm:text-[.9em] md:text-lg' ><span className='max-sm:hidden'>Rs</span> {item.quantity * Number(item.price.replace(/,/g, ''))} </p>
                <img onClick={() => deleteFromCart(item)} src="/images/delete.png" alt="" className='block cursor-pointer size-8 object-cover' />
        ...
        <section className='w-[20em] new:w-[28%] bg-[#FBEBB5] h-[18em] rounded-lg py-[.6em] md:py-[1.5rem] flex flex-col items-center gap-[4px] px-[.8rem] md:px-[1.7rem]'>
          ...
          <br />
          <Link href='/CheckOut'>
            <Button title={'Check it out'} btnClass={'text-[1em] px-3 py-2 md:p-4 border border-black rounded-lg text-[1em] lg:text-xl font-medium '} />
          </Link>
        </section>
      </div>
      <Gurantees />
